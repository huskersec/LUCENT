...
**Next block (pick up here):** **M3a VM run** — the patch-diff floor is BUILT (`bench/run_agent_m3.py` + the `diff` tool + the `vuln.exe`/`patched.exe` pair). It needs the M3 toolchain in the image first: re-run `setup-vm.ps1` (now installs JDK 21 + Ghidra + ghidriff), confirm the M3 probes green, **re-snapshot `golden`**, then `python bench\run_agent_m3.py` for the walkable diff→verdict trajectory. After that: **M3b** (MCP-client layer + GhidraMCP + WinDbg-MCP). Deferred: TTD `reached_sink` leg, `trial.py` file→argv retarget, host→guest sync workflow.
